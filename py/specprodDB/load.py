...
def get_options(description="Load redshift data into a specprod database."):
    ...
    prsr.add_argument('-z', '--zcatalog', action='store_true',
                      help='Data files were generated by zcatalog_merge.')
    prsr.add_argument('datapath', metavar='DIR', help='Load the data in DIR.')
    options = prsr.parse_args()
    return options


def find_files(specprod, config, options):
    """Identify the locations of files to be loaded.

    Parameters
    ---------
    specprod : :class:`str`
        The name of the specprod to be loaded.
    ...
    -------
    :class:`dict`
        A mapping of table name to file name(s).
    ...
    if specprod == 'daily':
        ...
        tiles_type = 'fits'
    filepaths = dict()
    filepaths['tile'] = os.path.join(options.datapath, 'spectro', 'redux',
                                     specprod, f'tiles-{specprod}.{tiles_type}')
    filepaths['exposure'] = os.path.join(options.datapath, 'spectro', 'redux',
                                         specprod, f'exposures-{specprod}.fits')
    filepaths['frame'] = filepaths['exposure']
    if options.zcatalog:
        filepaths['photometry'] = glob.glob(os.path.join(options.datapath,
                                                         f'{specprod}.photometry.*.fits'))
        filepaths['target'] = glob.glob(os.path.join(options.datapath, f'{specprod}.target.*.fits'))
        filepaths['ztile'] = glob.glob(os.path.join(options.datapath, f'{specprod}.ztile.*.fits'))
        filepaths['zpix'] = glob.glob(os.path.join(options.datapath, f'{specprod}.zpix.*.fits'))
        filepaths['fiberassign'] = glob.glob(os.path.join(options.datapath, f'{specprod}.fiberassign.*.fits'))
    else:
        filepaths['photometry'] = glob.glob(os.path.join(options.datapath, 'vac',
                                                         release, 'lsdr9-photometry',
                                                         specprod, photometry_version,
                                                         'potential-targets', 'tractorphot',
                                                         'tractorphot*.fits'))
        if target_summary:
            filepaths['target'] = os.path.join(options.datapath, 'vac', release, 'lsdr9-photometry', specprod, photometry_version, 'potential-targets', f'targetphot-potential-{specprod}.fits')
        else:
            filepaths['target'] = glob.glob(os.path.join(options.datapath, 'vac', release, 'lsdr9-photometry', specprod, photometry_version, 'potential-targets', 'targetphot-potential-*.fits'))
        ...
        if redshift_type == 'base' or redshift_type == 'patch':
            ...
            filepaths['zpix'] = os.path.join(redshift_dir, f'zall-pix-{specprod}.fits')
            filepaths['ztile'] = os.path.join(redshift_dir, f'zall-tilecumulative-{specprod}.fits')
            if not os.path.exists(filepaths['zpix']):
                log.warning("%s not found, will use individual survey-program files.")
                if redshift_version == 'v2':
                    filepaths['zpix'] = glob.glob(os.path.join(redshift_dir, '*', f'zpix-*.fits'))
                else:
                    filepaths['zpix'] = glob.glob(os.path.join(redshift_dir, f'zpix-*.fits'))
            if not os.path.exists(filepaths['ztile']):
                log.warning("%s not found, will use individual survey-program files.")
                if redshift_version == 'v2':
                    filepaths['ztile'] = glob.glob(os.path.join(redshift_dir, '*', f'ztile-*-cumulative.fits'))
                else:
                    filepaths['ztile'] = glob.glob(os.path.join(redshift_dir, f'ztile-*-cumulative.fits'))
        elif redshift_type == 'zcat':
            redshift_dir = os.path.join(options.datapath, 'vac', release, 'zcat', specprod)
            filepaths['zpix'] = os.path.join(redshift_dir, redshift_version, f'zall-pix-{release}-vac.fits')
            filepaths['ztile'] = os.path.join(redshift_dir, redshift_version, f'zall-tilecumulative-{release}-vac.fits')
        elif redshift_type == 'daily':
            redshift_dir = os.path.join(options.datapath, 'spectro', 'redux', specprod)
            filepaths['zpix'] = 'Daily loads are not meant to include Healpix redshifts.'
            filepaths['ztile'] = 'Only cumulative redshifts are needed for daily loads.'
        else:
...
def main():
    ...
    loaders = {'exposures': [{'filepaths': file_data['tile'],
                             ...
                             {'filepaths': file_data['exposure'],
                              'tcls': schemamodule.Exposure,
                              'hdu': 'EXPOSURES',
                              'q3c': 'tilera',
                              'chunksize': chunksize
                              },
                             {'filepaths': file_data['frame'],
               ...
               #
               'photometry': [{'filepaths': file_data['photometry'],
               ...
               #
               'targetphot': [{'filepaths': file_data['target'],
                               'tcls': schemamodule.Photometry,
                               'hdu': 'TARGETPHOT',
               ...
               'target': [{'filepaths': file_data['target'],
                           'tcls': schemamodule.Target,
                           'hdu': 'TARGETPHOT',
                           'chunksize': chunksize
                           }],
               'redshift': [{'filepaths': file_data['ztile'],
               ...
               'fiberassign': [{'filepaths': file_data['fiberassign'],
                               ...
                               {'filepaths': file_data['fiberassign'],
    ...
    if specprod != 'daily':
        loaders['redshift'].append({'filepaths': file_data['zpix'],
    ...
    if options.zcatalog:
        if options.load == 'fiberassign':
            del loader[0]['row_filter']
            del loader[1]
        elif options.load == 'redshift':
            del loader[0]['row_filter']
            del loader[1]['row_filter']
        else:
            pass
    #
    ...
    #
    # Potential future hook for generating zcatalog tables from individual
    # survey/program files.
    #
    # if options.load == 'redshift':
    #     if isinstance(loader[0]['filepath'], list):
